...
//    	$("#menuContinenti").prop('disabled', function (_, val) { return ! val; });
	...
	_openNode: function(node) {
		var self = this;
		var context = {};
		console.log("_openNode object after: %O", session.user_context);
        if(node.data.odoo_model == "muk_dms.file") {
            console.log('node.data.odoo_model == "muk_dms.file"');
    		context = $.extend(session.user_context, {
                form_view_ref: 'mvb_document_directory.mvb_view_dms_file_form'
            });
    	}else if (node.data.odoo_model == "muk_dms.directory")
    	{
            if (session.user_context.form_view_ref == 'mvb_document_directory.mvb_view_dms_file_form')
            {
                delete session.user_context.form_view_ref;
            }
    	}
    	console.log("_openNode object before: %O", session.user_context);
		this.do_action({
...
//             target: this.params.action_open_dialog ? 'new' : 'current',
    ...
    _editNode: function(node) {
		var self = this;
		var context = {};
		console.log("_editNode object after: %O", session.user_context);
        if(node.data.odoo_model == "muk_dms.file") {
    		context = $.extend(session.user_context, {
                form_view_ref: 'mvb_document_directory.mvb_view_dms_file_form'
            });
...
//             target: this.params.action_open_dialog ? 'new' : 'current',
    ...
    _createNode: function(node, type) {
        console.log('Thanhnd: _createNode')
		var self = this;
    	var context = {};
    	if(type == "muk_dms.file") {
            ...
            var form_view_ref = 'mvb_document_directory.mvb_view_dms_file_form'
            Object.assign(session.user_context, {form_view_ref});
...
//             target: this.params.action_open_dialog ? 'new' : 'current',
    ...
    _deleteNode: function(ev) {
        console.log('thanhnd: _deleteNode')
    	var self = this;
    	var node = ev.data.node;
    ...
    _deleteNodes: function(nodes) {
        console.log('thanhnd: _deleteNodes')
    	var self = this;
    ...
    _renameNode: function(ev) {
        console.log('thanhnd: _renameNode')
    	var self = this;
    	var node = ev.data.node;
		this._rpc({
            model: node.data.odoo_model,
    ...
    _mydownload: function(node) {
		return this._rpc({
    				 args: [node.data.odoo_id],
    				 model: 'muk_dms.file',
    				 method: 'action_groups_folder_download_attachment',
    				 context: session.user_context,
    			}).then(function(records) {
                    framework.blockUI();
                    session.get_file({
